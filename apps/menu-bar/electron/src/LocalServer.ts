...
export class LocalServer {
  ...
  setupRoutes() {
    this.app.get('/tlink/status', (_, res) => {
      res.json({ ok: true, version: electronApp.getVersion() })
    })

    this.app.get('/tlink/open', (req, res) => {
      ...
      const deeplinkURL = urlParam.replace('https://', 'tlink://').replace('exp://', 'tlink://')
  ...
  extractRootDomain(urlString: string) {
    try {
      ...
      if (!hostName) {
        // Tray link deeplink may include specific routes in the URL e.g. /update, /snack, /download, etc.
