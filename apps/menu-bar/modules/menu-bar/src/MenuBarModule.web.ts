...
const MenuBar = requireElectronModule<ElectronMainMenuBarModule & ElectronPreloadMenuBarModule>('MenuBar')

class EventEmitter {
  addListener<T>(eventName: string, listener: (event: T) => void) {
    return MenuBar.addListener(eventName, (data: unknown) => {
      listener(data as T)
    })
  }
}
...
export default {
  ...MenuBar,
  async runCli(...args: Parameters<typeof MenuBar.runCli>) {
    try {
