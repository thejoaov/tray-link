...
const migratePreferencesIfNeeded = (legacySettings?: LegacySettings): boolean => {
  ...
  if (current.defaultTerminal === defaultUserPreferences.defaultTerminal && legacyDefaultTerminal) {
...
export const getLegacyMigrationPreview = async (): Promise<LegacyMigrationPreview | null> => {
  ...
  const hasSettings = Boolean(
    root.settings?.locale || root.settings?.defaultEditor?.command || root.settings?.defaultTerminal?.command,
  )
