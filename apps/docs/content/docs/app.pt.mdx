...
Quando o modo de seleção está ativo, escolher um editor ou terminal salva essa ferramenta como padrão apenas daquele projeto, em vez de abri-la imediatamente. O override selecionado para o projeto é marcado com `*` no menu. Se o projeto não tiver override, o Tray Link usa os padrões globais definidos em Configurações.

Se **Mostrar ícones reais dos apps** estiver habilitado nas Configurações, os botões rápidos de editor e terminal na lista de projetos usam o ícone efetivo da ferramenta daquele projeto.
...
Esse é o fallback global usado sempre que um projeto não define seu próprio editor padrão.
...
Esse é o fallback global usado sempre que um projeto não define seu próprio terminal padrão.
